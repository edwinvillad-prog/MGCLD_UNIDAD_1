...
with tabs[2]:
    ...
    if data is not None:
        ...
        if dist_choice == "Normal":
            ...
            # ➕ Prueba Lilliefors (ajuste a parámetros estimados)
            stat_lillie, pval_lillie = lilliefors(x, dist='norm')
            have_lillie = True
            ...
            # ➕ Prueba Lilliefors (ajuste a parámetros estimados)
            stat_lillie, pval_lillie = lilliefors(x, dist='exp')
            have_lillie = True
        ...
        # =========================
        # 1) Normalizamos decisiones booleanas por prueba (rechazo = True)
        ...
        # 2) Definimos “pruebas primarias” según contexto científico
        primary_results = []  # tupla (nombre, reject_bool, detalle_str)

        if dist_choice == "Normal":
            if n < 20 and have_shap:
                primary_results.append(("Shapiro–Wilk", shap_reject,
                                        f"p={pval_shap:.4f}"))
            else:
                if have_ad:  # n ≥ 8 por construcción
                    primary_results.append(("Anderson–Darling", ad_reject,
                                            "comparación con valor crítico"))
                if have_lillie is True:
                    primary_results.append(("Lilliefors", lillie_reject,
                                            f"p={pval_lillie:.4f}"))
                # KS clásico queda como respaldo
            ...
            # Weibull / Gamma / Lognormal → KS clásico como primario
            primary_results.append(("KS clásico", ks_reject, f"p={pval:.4f}"))

        # 3) Aplicamos el criterio elegido en el sidebar
        st.markdown("---")
        st.subheader("🧾 Dictamen final")

        if decision_mode_cont.startswith("Estricto"):
            # Industrial: rechazar si ≥1 primaria rechaza
            ...
            # Docente: prioriza KS/Lilliefors; AD es complementaria
        ...
        # 4) Nota de discrepancias (transparencia)
        flags = []
        if have_ad is True and ad_reject is not None and ad_reject != ks_reject:
